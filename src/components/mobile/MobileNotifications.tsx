...
export function MobileNotifications() {
  ...
  const activeKind = KIND_FILTER[tab];
  const filtered = NOTIFICATIONS.filter((n) => activeKind === null || n.kind === activeKind);
  ...
  return (
    <div style={{ minHeight: '100dvh', background: 'var(--bg)', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ display: 'flex', gap: 6, padding: '12px 14px', borderBottom: '1px solid var(--line)', overflowX: 'auto' }} className="noscroll">
        {TABS.map((t) => {
          const kind = KIND_FILTER[t];
          const count = NOTIFICATIONS.filter((n) => !n.read && (kind === null || n.kind === kind)).length;
          return (
